...
- [X] T109 Dialect boundary before UC03: glareshield indicator and home "not offered" derive from `available: false` in the capability map (`useUnavailableOperations`); `instance.limited` removed from the session and contract (`dialect`, `apiVersion` kept as diagnostics); `NativeNamespaces` and `limited.spec.ts` decide by availability; `AdminClient` no longer reads `apiVersion`; gate `frontend/scripts/check-dialect-boundary.mjs` (`npm run check:dialect`, part of `build`) with declared exceptions (`Dialect`, `V1Translations`, `API/Session.cls` diagnostics, generated `OpenAPI.cls`, `e2e/setup/users.ts`); `Installer.CheckVersion` unchanged (bootstrap)
